...
(function(g, h) {
	...
	var a = {
		...
		k.basePath = (function() {
			...
			if (p == null) {
				p = "http://static.gensee.com/webcast/static/sdk/"
			}
			return p
		})();
		...
		var j = (function() {
			...
			l.fn = l.prototype = {
				...
				init: function() {
					...
					if (g.location.protocol && g.location.protocol.indexOf("https") == 0) {
						o.baseURL = o.baseURL.replace("http:", "https:")
					}
				...
				fire: function(q) {
					...
					if (q.type == "onRewardResponse" && q.data && q.data.userControl != h && q.data.userControl == "false" && q.data.returnCode == "0") {
						g.top.location.href = q.data.codeUrl;
						return
					}
				...
				load: function() {
					...
					var m = this;
					m.elem.innerHTML = '<span style="background:url(' + k.basePath + '/images/loading1.gif) no-repeat;height:18px;padding:0 0 2px 20px;">Loading...</span>';
		...
		var i = (function() {
			...
			l.fn = l.prototype = {
				...
				removeWidget: function(p) {
					if (typeof p === h) {
						throw new ERROR("widgetId is undefined.")
					}
					var q = this.widgets[p];
					if (k.isEmpty(q)) {
						throw new ERROR("widget not found in suite for widgetId:" + p)
					}
					q.suite = null;
					for (var o in this.eventsmap) {
						var m = this.eventsmap[o];
						if (m) {
							for (var n = 0; n < m.length; n++) {
								if (m[n]["wgid"] == p) {
									m.splice(n, 1);
					...
					delete this.widgets[p];
					delete k.widgets[p];
					k.log("Widget(" + p + ") has remove!", "INFO", p);
					return q
				}
			};
			return l
		...
		return function(l) {
			...
			if (k.proxy === h) {
				...
				j.send = function(o, r, n, q) {
					...
					o = i.trim(o);
					o = b.types(o);
					var p = new i.Event(o, k.eventTargetProxy, r, n, q);
					var m = k.fire(p);
					if (m == 0) {
						var s = k.suite.key == i.defaultKey ? "" : k.suite.key + ".";
						i.log("No handler had bound to '" + s + p.type + "'!", "WARN", p)
					}
					return m
				};
	...
	e.gsmlTagHandler = function(l, n) {
		...
		for (var k = 0; k < p.length; k++) {
			...
			if (e.isEmpty(j)) {
				e.log("Tag's attr -" + p[k] + " is required.", "ERROR")
			}
		}
		var o = n.getAttribute("group");
		var m = e.suite(o);
		m.createWidget("GSML", l, {
			elem: n
		}).load()
	};
	e.gsmlTagUninstallHandler = function(j) {
		if (e.isEmpty(j)) {
			return
		}
		var k = e.widgets[j];
	...
	var c = g.GS = {
		...
		unloadTag: function(i) {
			e.gsmlTagUninstallHandler(i)
		},
	...
	e.api.ready(function() {
		if (c.autoload) {
			f.loadTags(["video-live", "video-vod", "audio-live", "audio-vod", "doc", "video-host", "video-pub", "wb", "doc-thumb", "detect-tool", "gs-upload", "gs-barrage"], e.gsmlTagHandler)
		}
	});
